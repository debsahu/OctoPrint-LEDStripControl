...
phy_to_bcm = { 0:None, 1:None, 2:None, 3:2, 4:None, 5:3, 6:None, 7:4, 8:14,9:None, 10:15, 11:17, 12:18, 13:27, 14:None, 15:22, 16:23, 17:None, 18:24, 19:10, 20:None, 21:9, 22:25, 23:11, 24:8, 25:None, 26:7, 27:0, 28:1, 29:5, 30:None, 31:6, 32:12, 33:13, 34:None, 35:19, 36:16, 37:26, 38:20, 39:None, 40:21 }
...
class PiBlasterPin(object):
        def __init__(self, pin, logger):
                ...
                if phy_to_bcm.get(pin) is not None:
                        ...
                        self._pin = pin
                        self._logger.debug(u"PiBlasterPin: coverted pin: %r to %r" % (pin, self._pin))

        def _write_to_dev(self, cmd):
                if not os.path.exists(self._dev):
                        raise Exception(u"%s does not exist" % self._dev)
                if not os.access(self._dev, os.W_OK):
                        raise Exception("%s is not writable" % self._dev)
                self._logger.debug(u"PiBlasterPin:_write_to_dev(%s)" % cmd)
        ...
        def ChangeDutyCycle(self, dutycycle):
                self._logger.debug(u"PiBlasterPin: ChangeDutyCycle() pin: %s" % self._pin)
...
class LEDStripControlPlugin(octoprint.plugin.AssetPlugin,
        ...
        def _setup_pin(self, pin):
                ...
                if pin:
                        ...
                        elif self._settings.get_boolean(['pigpiod']):
        ...
        def _unregister_leds(self):
                ...
                if not self._settings.get_boolean(['pigpiod']) and not self._settings.get_boolean(['piblaster']) and GPIO:
                        GPIO.cleanup()
                self._leds = dict(r=None, g=None, b=None)
        ...
        def get_settings_defaults(self):
                return dict(r=0, g=0, b=0, pigpiod=False, piblaster=False)
